...
export default function ResetPasswordPage() {
  ...
  // 🆔 บัตรประชาชนเพิ่ม States ใหม่สำหรับเช็คข้อมูล
  ...
  const handleSetValue = (key: string) => (val: string) => {
    ...
    // ล้างสถานะการตรวจสอบเดิมหากมีการพิมพ์แก้ไขใหม่
    if (key === "identifier") {
  ...
  if (!result.isValid) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // สมมติว่าหลังบ้านส่งมาในรูปแบบ { first_name: "สมชาย", last_name: "ใจดี" }
        const fullName = `${data.first_name || ""} ${data.last_name || ""}`.trim();
        setVerifiedFullName(fullName || "ไม่พบชื่อในระบบพนักงานแต่เลขถูกต้อง");
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!finalIdentifier) {
      ...
      if (finalIdentifier.length !== 13) {
        ...
        errs.identifier = "กรุณากดปุ่มตรวจสอบเลขบัตรประชาชนให้ผ่านก่อน";
      }
    }
      ...
      <div style={{
        ...
        <div style={{ display: "flex", borderRadius: 12, padding: 4, marginBottom: 22, background: "#f0faf5" }}>
          {(["user", "staff"] as const).map(r => (
            <button key={r} type="button" onClick={() => { setRole(r); setFieldErrors({}); setIsEmailVerified(false); setIsIdCardVerified(false); setVerifiedFullName(""); }}
              style={{
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Field label="เลขบัตรประชาชน หรือ อีเมล" error={fieldErrors.identifier}>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {/* ✅ ปุ่มกรณีพิมพ์ข้อมูลเป็น บัตรประชาชนความยาวครบ 13 หลัก */}
              {isInputIdCard13 && !isIdCardVerified && (
                <button
                  ...
                  style={{
                    padding: "0 14px", borderRadius: 12, border: "none",
                    background: checkingIdCard ? "#fca5a5" : "#ef4444",  // 🔴 แดงเสมอก่อนผ่าน
                    color: "#fff", fontSize: 13, fontWeight: 600,
              ...
              {isInputIdCard13 && isIdCardVerified && (
                <button
                  ...
                  style={{
                    padding: "0 14px", borderRadius: 12, border: "none",
                    background: "#16a360",  // 🟢 เขียวหลังผ่าน
                    color: "#fff", fontSize: 13, fontWeight: 600,
                    cursor: "default",
                    whiteSpace: "nowrap",
                  }}
            ...
            {/* ✅ กล่องแสดงชื่อ-นามสกุลเมื่อกดยืนยันบัตรประชาชนผ่าน */}
            {isIdCardVerified && (
          ...
          <div style={{ padding: "10px 14px", borderRadius: 12, background: "rgba(22,163,96,0.06)", border: "1px solid rgba(22,163,96,0.15)", display: "flex", gap: 8, alignItems: "flex-start" }}>
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#4a7c62", lineHeight: 1.6 }}>
              ระบบจะตรวจสอบข้อมูลชื่อผู้ใช้งาน (Username) และเลขบัตรประชาชนหรืออีเมลที่ลงทะเบียนไว้ก่อนเปลี่ยนรหัสผ่านสำเร็จ
            </p>
          </div>
